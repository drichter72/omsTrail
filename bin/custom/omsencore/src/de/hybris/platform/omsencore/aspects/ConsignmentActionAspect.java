 ...
 *
 */
package de.hybris.platform.omsencore.aspects;
 ...
 *
 * @deprecated since 6.4. See {@link de.hybris.platform.warehousing.cancellation.ConsignmentCancellationService#processConsignmentCancellation(OrderCancelResponse)}.
 */
@Deprecated
public class ConsignmentActionAspect implements AfterAdvice
{
		...
		for (ConsignmentModel consignment : getConsignments(joinPoint))
		{
			...
			try
			{
				getConsignmentBusinessProcessService()
						.triggerChoiceEvent(consignment, OmsencoreConstants.CONSIGNMENT_ACTION_EVENT_NAME,
								getChoice());
			}
	...
	protected List<ConsignmentModel> getConsignments(final JoinPoint joinPoint)
	{
		...
		return orderCancelResponse.getOrder().getConsignments().stream().filter(
				consignment -> consignment.getStatus().equals(ConsignmentStatus.CANCELLED) && !consignment.getConsignmentProcesses()
						.iterator().next().isDone()).collect(Collectors.toList());
	}
